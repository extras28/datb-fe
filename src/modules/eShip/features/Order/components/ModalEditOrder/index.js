...
import { thunkGetListOrder } from '../../orderSlice';
...
function ModalOrderEdit(props) {
  ...
  const { show, onClose, onRefreshOrderList, orderItem, onExistDone, employees } = props;
  ...
  async function requestCreateOrder(values) {
    try {
      ...
      if (result == 'success') {
        ...
        dispatch(thunkGetListOrder(Global.gFiltersOrderList));
        handleClose();
      }
    } catch (error) {
      console.log(error);
